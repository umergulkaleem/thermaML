...
export const ThermaMLProvider = ({ children }) => {
  ...
  const [selectedDate, setSelectedDate] = useState('2023-07-20');
  
  const [availableTiles, setAvailableTiles] = useState([]);
  const [selectedTileId, setSelectedTileId] = useState(426);
  ...
  const handleConfigChange = (mockMode, newUrl) => {
    ...
    showToast(mockMode ? "Switched to Built-in Mock Engine" : `Switched to Live REST API (${newUrl})`, 'info');
  };
    ...
    async function initDiscovery() {
      try {
        ...
        if (isMounted) {
          ...
          // Default to high summer date if available
          const defaultDate = dates.includes('2023-07-20') ? '2023-07-20' : dates[0];
    ...
    async function fetchTilesForDate() {
      try {
        ...
        if (isMounted) {
          ...
          const tileExists = tiles.some(t => Number(t.tile_id) === Number(selectedTileId));
  ...
  const fetchSinglePrediction = useCallback(async (model = selectedModel, date = selectedDate, tileId = selectedTileId) => {
  ...
  const fetchModelComparison = useCallback(async (date = selectedDate, tileId = selectedTileId) => {
  ...
  const fetchScenarioSimulation = useCallback(async (
    model = selectedModel,
    date = selectedDate,
    ...
    try {
      ...
      setError(null);
      const res = await predict_scenario_for_date_tile(model, date, tileId, scenario);
  ...
  }, [selectedModel, selectedDate, selectedTileId, scenarioInputs, useMock, baseUrl]);

  // Automatically refresh views when primary context changes
  useEffect(() => {
  ...
  }, [selectedDate, selectedTileId, selectedModel, fetchSinglePrediction, fetchModelComparison, fetchScenarioSimulation]);

  // Get active tile metadata object
  const activeTileMeta = availableTiles.find(t => Number(t.tile_id) === Number(selectedTileId)) || null;

  return (
    <ThermaMLContext.Provider value={{
      // Configuration
      useMock,
      baseUrl,
      handleConfigChange,
